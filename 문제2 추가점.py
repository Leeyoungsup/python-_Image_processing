import PIL.Image as pilimg
import numpy as np
import matplotlib.pyplot as plt
import math
pi=3.14
hand= np.array(pilimg.open('_7_8_1.jpg'))
edge=np.array(np.where(hand[:,:,1]>150))
wcx=(np.max(edge[0])+np.min(edge[0]))/2
wcy=(np.max(edge[1])+np.min(edge[1]))/2
a=int(edge.size//2)
th=np.zeros((a))
d=np.zeros((a))
do=np.zeros((360))
the=np.arange(360)
for i in range(a):
    d[i]=math.sqrt((edge[0,i]-wcx)**2+(edge[1,i]-wcy)**2)
    th[i]=math.atan2(-edge[1,i]+wcy,-edge[0,i]+wcx)*180/pi+180
    q=int(round(th[i]))
    if do[q]<=d[i]:
        do[q]=d[i]
for s in range(1,359):
    if do[s]==0:
        g,h=s+1,s-1
        while(1):
            if h!=0 and do[h]==do[h-1]:
                h=h-1
            else:
                break
        while(1):
            if g!=359 and do[g]==do[g+1]:    
                g=g+1
            else:
                break
        if h!=0 and g!=359:
            do[s]=((do[h]-do[g])/(h-g))*(s-h)+do[h]
for s in range(1,359):
    do[s]=(do[s-1]+do[s+1])/2
for k in range(1,359):
    if do[k]>=do[k-1] and do[k]>=do[k+1]:
        h,g=k,k
        while(1):
            if h!=0 and do[h]>do[h-1]:
                h=h-1
            else:
                break
        while(1):
            if g!=359 and do[g]>do[g+1]:    
                g=g+1
            else:
                break
        if do[h]>=10 and do[g]>=10 and do[k]-do[h]>=5 and (do[k]-do[g])>=5:
            reddot=np.array(([do[k]],[k]))
            plt.plot(reddot[1],reddot[0],'ro',markersize=12)       
plt.plot(the,do,'b-')
plt.show()
